...
fn main() {
    ...
    let target_dir = Path::new(&target_dir_env).join("target-tests");
    let ws = Path::new("..").join("..");
    let ws_target = ws.join(&target_dir);
    let in_wasm = ws_target.join("wasm32-unknown-unknown").join("release").join("canister-a.wasm");
    let out_dir_env = std::env::var("OUT_DIR").unwrap();
    let out_wasm = Path::new(&out_dir_env).join("canister-a.wasm");
